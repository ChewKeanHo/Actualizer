...
# INITIALIZATION
# ==============
# Initialize bash sequences from system-level to user-level
...
if [ "${chroot:-}" = "" ] && [ -r /etc/debian_chroot ]; then
        chroot="$(cat /etc/debian_chroot)"
fi

# set hostname if unavailable
get_hostname() { \
...
# ====================
if [ "$USER_ACTUAL" = "0" ]; then
        PS1="${chroot:+($chroot)}ROOT@${HOSTNAME}:\w\n\$ "
else
        PS1="${chroot:+($chroot)}\u@${HOSTNAME}:\w\n\$ "
fi
...
PATH="${PATH}:${HOME}/.local/bin:${HOME}/.local/sbin/:${HOME}/bin:${HOME}/sbin"
...
# UNIX Compatibilities
# ====================
unalias pbcopy &> /dev/null
unalias pbpaste &> /dev/null
...
if [ $? -ne 0 ]; then
        sudo() {
                su root --preserve-environment --command "$*"
                return $?
        }
fi
